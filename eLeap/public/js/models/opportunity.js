...
define(['jquery', 'underscore', 'backbone', 'eLeap', 'controllers/restServer'], function($, _, Backbone, eLeap, server) {
	'use strict';
...
>>>>>>> c440dbccfda4d21417b1625324c61852923d751f
		...
		parseOppFromDB : function(dbOpportunity) {
			...
			if (jsonOpp.applicationDueDate)
				jsonOpp.applicationDueDate = new Date(jsonOpp.applicationDueDate);
			if (jsonOpp.availableSeats)
				jsonOpp.availableSeats = Number(jsonOpp.availableSeats);
			if (jsonOpp.classId)
				jsonOpp.classId = Number(jsonOpp.classId);
			if (jsonOpp.classYear)
				jsonOpp.classYear = Number(jsonOpp.classYear);
			if (jsonOpp.createDate)
				jsonOpp.createDate = new Date(jsonOpp.createDate);
			if (jsonOpp.donation)
