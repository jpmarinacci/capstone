...
define(['jquery', 'underscore', 'backbone', 'eLeap', 'controllers/restServer'], 
	...
	eLeap.own.Opportunity = Backbone.Model.extend({
		...
		parseOppFromDB: function(dbOpp) {
			...
			if(dbOpp.classId)					jsonOpp.classId = Number(dbOpp.classId);
			...
			if(dbOpp.numberOfTeams)				jsonOpp.numTeams = Number(dbOpp.numberOfTeams);
			...
			if(dbOpp.ownerName)					jsonOpp.ownerName = dbOpp.ownerName;
			if(dbOpp.payAmount)					jsonOpp.payAmount = Number(dbOpp.payAmount);
			...
			if(dbOpp.section)					jsonOpp.section = dbOpp.section;
