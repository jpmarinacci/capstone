...
IF(joinedCount IS NULL || joinedCount = 0) THEN
	...
	SELECT * , 0 AS 'availableSeats' FROM eLeapData.opportunity 
	WHERE opportunity.opportunityId = oppid_v;
    
    SELECT 0 AS 'personJoined';
ELSE
	SELECT 'success' AS 'status';
    /* calculated available seats*/
	SELECT * , opportunity.totalSeats-COUNT(filledSeatTable.opportunityId) AS 'availableSeats' FROM eLeapData.opportunity LEFT OUTER JOIN eLeapData.filledSeatTable ON opportunity.opportunityId = filledSeatTable.opportunityId
	...
	SELECT COUNT(filledSeatTable.opportunityId) AS 'personJoined' FROM eLeapData.filledSeatTable 
	WHERE filledSeatTable.opportunityId = oppid_v AND filledSeatTable.personId = personId_v;
